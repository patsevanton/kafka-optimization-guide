...
- При разделении (Partition tolerance) выбор делается между доступностью (Availability) и согласованностью (Consistency).
- При отсутствии разделения выбор делается между задержкой (Latency) и согласованностью (Consistency).

Настройки Kafka, такие как количество партиций, коэффициент репликации и параметры Продюсеров/потребителей, влияют на эти компромиссы.
