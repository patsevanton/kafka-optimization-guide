...
#### **Минимальное количество байт для получения (`fetch.min.bytes`)** и **максимальное время ожидания (`fetch.max.wait.ms`)** работают вместе:
- Уменьшение этих значений ускоряет отклик потребителя, но может снизить эффективность из-за более частых запросов.
- Увеличение этих значений может повысить задержку, но также увеличить эффективность обработки больших объёмов данных.

#### **Интервал автоматической фиксации (`auto.commit.interval.ms`)** влияет на надёжность и производительность:
- Уменьшение интервала повышает надёжность (чаще фиксируются смещения), но снижает пропускную способность.
- Увеличение интервала снижает надёжность, но может повысить производительность.

#### **Максимальное количество байт для выборки на раздел (`max.partition.fetch.bytes`)**:
- Уменьшение этого значения ниже стандартного (∼1 МБ) приводит к увеличению количества запросов, что может снизить производительность.
- Возврат к значению по умолчанию восстанавливает нормальное поведение потребителя.

#### **Коэффициент репликации напрямую влияет на задержку**.  
   При коэффициенте репликации = 1 средняя задержка составила **1,63 мс**. После увеличения коэффициента репликации до 3 задержка возросла до **2,27 мс**. Это связано с необходимостью синхронизации данных между несколькими репликами перед подтверждением записи.


#### **Измерение задержки — важный этап настройки Kafka**.  
   Инструменты типа `End-to-End Latency` помогают оценить производительность системы при разных конфигурациях и выбрать оптимальные параметры для конкретного use case.


#### Оптимизация сетевых параметров
   Оптимизация сетевых параметров (num.io.threads,socket.request.max.bytes,socket.send.buffer.bytes,replica.socket.receive.buffer.bytes,socket.receive.buffer.bytes) и потоков обработки позволяет значительно повысить пропускную способность кластера Kafka (на 30% в данном случае) при минимальном увеличении задержки, что является эффективным trade-off для scenarios, требующих высокой производительности передачи данных.
