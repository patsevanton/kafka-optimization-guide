...
## Глава 4. Оптимизация производителя

Производители Kafka отвечают за отправку сообщений в кластер Kafka. Их конфигурация существенно влияет на общую пропускную способность системы и задержку.
...
* **Время публикации** — промежуток времени с момента, когда внутренний производитель Kafka отправляет пакет сообщений брокеру, до момента, когда эти сообщения добавляются в журнал репликации лидера.
...
Объединение нескольких сообщений в пакет перед отправкой на брокеры Kafka снижает нагрузку и повышает пропускную способность. Параметр **batch.size** определяет максимальный размер пакета в байтах, а **linger.ms** указывает, как долго производитель будет накапливать сообщения перед отправкой пакета.

Увеличение размера пакета приводит к повышению пропускной способности, но также может увеличить задержку, поскольку производитель ожидает накопления достаточного количества сообщений для заполнения пакета ([Confluent Docs][12]).

**max.inflight.requests.per.connection** — определяет количество пакетов сообщений, которые производитель может отправить без получения ответов. Чем выше значение, тем выше пропускная способность, но больше расходуется памяти.
...
* Тюнинг производителей Kafka ([Confluent Docs][12])
