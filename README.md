...
**max.inflight.requests.per.connection** — определяет количество пакетов сообщений, которые продюсер может отправить без получения ответов. Чем выше значение, тем выше пропускная способность, но больше расходуется памяти.
